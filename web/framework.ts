...
const header = (...children: HTMLElement[]) =>
  $('header').$children(...children)
const content = (...children: HTMLElement[]) =>
  $('div')
    .$class('content')
    .$children(...children)
const footer = (...children: HTMLElement[]) =>
  $('footer').$children(...children)
const text = (text: string) => $('p').$text(text)
const createApp = (parent: HTMLElement, content: HTMLElement) =>
...
createApp(
  body,
  content(
    header(button('Page 1', toPage(0)), button('Page 2', toPage(1))),
    (myRouter = router(
      $('div').$children($('h2').$text('page 1'), text(lorem)),
      $('div').$children($('h2').$text('page 2'), text(lorem))
    )),
